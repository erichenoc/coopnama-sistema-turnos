  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Analisis de Sentimiento</h1>
          <p className="text-gray-500">Tendencias de satisfaccion del cliente</p>
        </div>
        <select
          value={days}
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <Card className="shadow-neu-sm">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-gray-500">{overview.avgRating}/5 Promedio</p>
            </CardContent>
          </Card>
          <Card className="shadow-neu-sm">
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-green-600">{sentimentPercent(overview.positive, overview.total)}%</p>
              <p className="text-xs text-gray-500">Positivo ({overview.positive})</p>
            </CardContent>
          </Card>
          <Card className="shadow-neu-sm">
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-gray-500">{sentimentPercent(overview.neutral, overview.total)}%</p>
              <p className="text-xs text-gray-500">Neutral ({overview.neutral})</p>
            </CardContent>
          </Card>
          <Card className="shadow-neu-sm">
            <CardContent className="p-4 text-center">
              <p className="text-3xl font-bold text-red-600">{sentimentPercent(overview.negative, overview.total)}%</p>
              <p className="text-xs text-gray-500">Negativo ({overview.negative})</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      {overview && overview.total > 0 && (
        <Card className="shadow-neu mb-8">
          ...
          <CardContent>
            {byAgent.length === 0 ? (
              ...
              <div className="space-y-3">
                {byAgent.map((agent) => (
                  <div key={agent.agentId} className="flex items-center gap-3 p-3 bg-gray-50 rounded-neu-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate">{agent.agentName}</p>
                      <p className="text-xs text-gray-500">{agent.total} evaluaciones</p>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="flex gap-1 text-xs">
                        ...
                        <span className="text-gray-500">{agent.neutral}</span>
                        <span className="text-gray-400">/</span>
            ...
            {byService.length === 0 ? (
              ...
              <div className="space-y-3">
                {byService.map((service) => (
                  <div key={service.serviceId} className="flex items-center gap-3 p-3 bg-gray-50 rounded-neu-sm">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 truncate">{service.serviceName}</p>
                      <p className="text-xs text-gray-500">{service.total} evaluaciones</p>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="flex gap-1 text-xs">
                        ...
                        <span className="text-gray-400">/</span>
                        <span className="text-gray-500">{service.neutral}</span>
                        <span className="text-gray-400">/</span>
